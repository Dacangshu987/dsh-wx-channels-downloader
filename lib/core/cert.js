...
import { caCertPath, caKeyPath, certsDir } from './paths.js';
...
function pfxPath(kind, host) {
    if (kind === 'ca')
        return `${certsDir()}/ca.pfx`;
    return `${certsDir()}/leaf-${host.replace(/[^a-z0-9.-]/gi, '_')}.pfx`;
}
/* ---------- validity metadata (sidecar json next to each pfx) ---------- */
function metaPathOf(pfx) {
    return `${pfx}.meta.json`;
}
function readNotAfter(pfx) {
    try {
        const t = Date.parse(JSON.parse(readFileSync(metaPathOf(pfx), 'utf8')).notAfter ?? '');
...
function writeNotAfterMeta(pfx, notAfter) {
    try {
...
async function notAfterOfCer(cerPath) {
    try {
...
function resolveNotAfter(pfx, legacyFallback) {
    return readNotAfter(pfx) ?? legacyFallback();
}
function isFresh(notAfterMs) {
...
export async function ensureCa() {
    ...
    if (existsSync(cert) && existsSync(pfx) && (await isCaInstalled())) {
        ...
        if (notAfter !== null)
            writeNotAfterMeta(pfx, notAfter);
        if (isFresh(notAfter))
            return;
...
export async function installToRoot(cerPath) {
    ...
    try {
        ...
        if (out.stderr && /failed|error/i.test(out.stderr))
