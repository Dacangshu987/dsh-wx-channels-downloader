...
export interface TaskState {
  ...
  username: string
  createtime: number
  state: 'pending' | 'downloading' | 'done' | 'skipped' | 'failed'
  progress: number
  path?: string
  error?: string
  startAt?: number
...
export function cleanFilename(name: string, maxLen = 120): string {
  let s = String(name ?? '')
...
export class Downloader {
  ...
  constructor(store: Store, downloadsDir: string, maxConcurrent: number, hub: Hub) {
    this.store = store
  ...
  snapshot(): { running: boolean; total: number; success: number; failed: number; skipped: number; tasks: TaskState[] } {
    let success = 0
    let failed = 0
    let skipped = 0
  ...
  enqueue(v: WxVideo, force = false): DownloadOutcome {
    if (!v.id || !v.url) return { started: false, skipped: false, message: 'missing id/url' }
    if (!force) {
  ...
  private setTask(id: string, patch: Partial<TaskState> & { title: string; nickname: string; username: string; createtime: number }): void {
    const prev = this.tasks.get(id) ?? { id, title: patch.title, nickname: patch.nickname, username: patch.username, createtime: patch.createtime, state: 'pending' as const, progress: 0 }
  ...
  private async runOne(v: WxVideo): Promise<void> {
    ...
    try {
      ...
      if (quality) name = `${name}_${quality}`
