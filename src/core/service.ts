...
import type { DownloadEntry, ProbeResult, ServerEvent } from '../shared/protocol.js'
import { ensureCa, ensureLeaf } from './cert.js'
import { Downloader } from './downloader.js'
import { Hub } from './events.js'
import { Injector } from './inject.js'
import { handleLocalApi, normalizeProfileToVideo } from './localApi.js'
import { MitmServer } from './proxy.js'
import { runProbe, setSystemProxy, type ProbeOutput } from './probe.js'
...
export interface ServiceDiagnostics {
  ...
  downloader: ReturnType<Downloader['snapshot']>
...
export class WxChannelsService {
  ...
  async start(): Promise<void> {
    ...
    this.proxy = new MitmServer(
      this.cfg.port,
      new Set([CHANNELS_HOST]),
      leaf,
      {
  ...
  async refreshProbe(emit = false): Promise<ProbeResult> {
    const out: ProbeOutput = await runProbe({
      port: this.cfg.port,
  ...
  private localApiContext() {
    return {
      ...
      shouldCapture: (username: string) => this.shouldCapture(username),
      saveCoverUrl: (coverUrl: string, videoId: string, title: string, author: string) => this.saveCoverUrl(coverUrl, videoId, title, author),
    }
  }

  /** HTML/JS rewrite seam used by the proxy. */
  private interceptResponse(host: string, pathname: string, contentType: string, body: Buffer): Buffer | null {
    ...
    try {
      ...
      this.hub.emitLog(`⚠️ 注入异常: ${(e as Error).message}`)
    }
    return null
  }

  async saveCoverUrl(coverUrl: string, videoId: string, title: string, author: string): Promise<string | null> {
  ...
  watchlistJson(): { username: string; nickname: string; enabled: number; captured: number; downloaded: number }[] {
  ...
  private shouldCapture(username: string): boolean {
  ...
  catalogJson(includeAll = false): unknown[] {
  ...
  async command(kind: string, payload: Record<string, unknown> = {}): Promise<{ ok: boolean; error?: string }> {
    try {
      switch (kind) {
        case 'runProbe':
          await this.refreshProbe(true)
        ...
        case 'download': {
          ...
          for (const row of rows) {
            if (ids.length && !ids.includes(row.object_id)) continue
            if (!this.inRange(row.createtime, rangeDays)) continue
            if (this.watchActive() && !this.store.isWatched(row.username)) continue
        ...
        case 'delete': {
          ...
          for (const id of ids) {
            ...
            if (rec) {
              if (rec.file_path) {
                try {
                  if (existsSync(rec.file_path)) rmSync(rec.file_path, { force: true })
                } catch {
        ...
        case 'addWatch': {
          ...
          if (!username && !nickname) return { ok: false, error: '需要博主标识（username 或昵称）' }
