...
import type { ProbeItem, ProbeResult } from '../shared/protocol.js'

const execFileAsync = promisify(execFile)
const INTERNET_SETTINGS = 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings'

async function readWinInetProxy(): Promise<{ enabled: boolean; server: string }> {
  try {
    const script = `
