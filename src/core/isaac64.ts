...
function isaac64Gen(ctx: RandCtx64): void {
...
export function createIsaacInst(encKey: bigint): RandCtx64 {
  const ctx: RandCtx64 = {
    randCnt: 255n,
    seed: new Array<bigint>(256).fill(0n),
  ...
  for (let i = 0; i < 256; i += 8) {
    ...
    mix(v)
    ctx.mm[i] = v[0]!; ctx.mm[i + 1] = v[1]!; ctx.mm[i + 2] = v[2]!; ctx.mm[i + 3] = v[3]!
    ctx.mm[i + 4] = v[4]!; ctx.mm[i + 5] = v[5]!; ctx.mm[i + 6] = v[6]!; ctx.mm[i + 7] = v[7]!
  }
...
function isaacRandom(ctx: RandCtx64): bigint {
...
export function decryptBuffer(data: Buffer, encLen: number, key: number | bigint): boolean {
  ...
  for (let i = 0; i < limit; i += 8) {
    ...
    for (let j = i; j < end; j++) data[j]! ^= block[j - i]!
  }
  return true
}

/** True when the 4-byte box type at offset 4 is not ftyp (i.e. the file is still encrypted). */
export function isEncryptedMp4(header: Buffer): boolean {
...
export function decryptFileInPlace(filePath: string, encLen: number, key: number | bigint): { ok: boolean; error?: string } {
  try {
