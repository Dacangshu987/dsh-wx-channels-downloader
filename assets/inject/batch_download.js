...
function __export_batch_video_list__() {
  ...
  // [wxdown 改造] 自动落库：把博主主页视频列表 POST 到后端，供应用探测全量列表（含 createtime）
  try {
    var _mgr = (typeof __wx_batch_download_manager__ !== 'undefined') ? __wx_batch_download_manager__ : {};
    var _author = exportData.length > 0 ? (exportData[0].nickname || '') : '';
